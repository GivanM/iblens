...
function startJob({ apiKey, anthropicVersion, payload }) {
  ...
  (async () => {
    try {
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "content-type": "application/json", "x-api-key": apiKey, "anthropic-version": anthropicVersion || "2023-06-01" },
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (req.method === "POST" && req.url === "/submit") {
      const { apiKey, anthropicVersion, payload } = JSON.parse((await readBody(req, 4 * 1024 * 1024)).toString("utf8"));
      if (!apiKey || !payload) return send(400, { error: "apiKey and payload required" });
      return send(200, { id: startJob({ apiKey, anthropicVersion, payload }) });
    }
    ...
    if (req.method === "POST" && commit) {
      ...
      const { apiKey, anthropicVersion, payload } = JSON.parse(zlib.gunzipSync(Buffer.concat(u.parts.slice(0, count))).toString("utf8"));
