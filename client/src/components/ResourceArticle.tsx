...
import { shortTitle, sentenceCase } from "@shared/pageNames";
...
export function ResourceArticle({
  ...
  // About and similar pages use this layout without being resources.
  const inResources = canonical.startsWith("/resources");
  return (
    <>
      <SEOHead
        title={title}
        ...
        breadcrumbs={[
          { name: "Home", url: "/" },
          ...(inResources ? [{ name: "Resources", url: "/resources" }] : []),
          { name: sentenceCase(name), url: canonical },
        ]}
      />
