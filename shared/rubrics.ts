...
const ECONOMICS_IA: Rubric = {
  ...
  notes: "Word limit 800 per commentary, moderators will not read beyond 800 words. The full portfolio is marked out of 45: three commentaries (14 each) + Criterion F: Rubric requirements (3): different units, different sources, articles no older than one year. Each commentary must use a different key concept. Diagrams do not come through pasted text: mark Criterion A on the diagrams the commentary identifies and explains, do not lower it because a diagram or its labels cannot be seen, and say in the comment that the diagrams were not seen and that levels 2 and 3 also need them to be accurate and correctly labelled, so the mark assumes they are. If the commentary refers to no diagram at all, mark Criterion A on that basis, since even the lowest level needs a relevant diagram, and say that no diagram was referred to so the student should check one is included.",
...
const MATH_IA: Rubric = {
  ...
  criteria: [
    ...
    { name: "Criterion D: Reflection", max: 3, descriptor: "Reflection that reviews, analyses and evaluates the exploration: meaningful reflection links to the aim, considers limitations or compares approaches; critical reflection, present throughout, discusses implications of results, strengths and weaknesses of approaches and what next (describing results is limited reflection)" },
...
const EXTENDED_ESSAY: Rubric = {
  ...
  wordLimit: { max: 4000, excludes: "citations and references, the bibliography, contents page, headers, tables, equations, charts and diagrams", stopsAt: true },
};

// How the TOK guide says its two holistic instruments are applied. Shared by the essay and the exhibition.
const HOLISTIC_RULE = "How to apply the levels, as the TOK guide sets out: they are global, holistic descriptors, not a checklist of necessary characteristics. Find the level whose descriptor conveys most accurately the level the work attains, reading it as a whole; not every aspect of a descriptor has to be met for a mark in that level. Award the upper mark of a level when the work shows its qualities to a great extent and the lower mark when it shows them to a lesser extent. If the work seems to fall between two levels, re-read the driving question and choose the level that better describes the overall impression: in the higher of the two award its lower mark, in the lower of the two its upper mark. The highest level does not imply faultless performance, and you should not hesitate to use the extremes. When you explain the mark, describe how the work as a whole matches that level; never say that one missing feature prevents a level or is required for it, except for the rules stated below (the object caps and the rule on prescribed titles or IA prompts).";
...
const TOK_ESSAY: Rubric = {
  ...
  notes: "Marked holistically against ONE instrument (no sub-criteria), guided by the driving question: \"Does the student provide a clear, coherent and critical exploration of the essay title?\" The level descriptors, as the TOK guide words them. Excellent 9-10: the discussion has a sustained focus on the title and is linked effectively to areas of knowledge; arguments are clear, coherent and effectively supported by specific examples; the implications of arguments are considered; there is clear awareness and evaluation of different points of view. Good 7-8: the discussion is focused on the title and is linked effectively to areas of knowledge; arguments are clear, coherent and supported by examples; there is awareness and some evaluation of different points of view. Satisfactory 5-6: the discussion is focused on the title and is developed with some links to areas of knowledge; arguments are offered and are supported by examples; there is some awareness of different points of view. Basic 3-4: the discussion is connected to the title and makes superficial or limited links to areas of knowledge; the discussion is largely descriptive; limited arguments are offered but they are unclear and are not supported by effective examples. Rudimentary 1-2: the discussion is weakly connected to the title; while there may be links to the areas of knowledge, any relevant points are descriptive or consist only of unsupported assertions. 0: the discussion does not reach the standard described by the other levels or is not a response to one of the prescribed titles for the correct examination session. Word limit 1,600. " + HOLISTIC_RULE + " Rules: the 2022 course has no ways of knowing, so never refer to them. Do not require a personal perspective, a named thinker, philosopher, scholar or theory, or a separately formulated knowledge question: none of these appears in the descriptors. A concept, theory, thinker or example the essay does not use may appear only as an optional suggestion ('you could'), never as a reason for the mark and never as the title of a risk. You cannot check the six prescribed titles for the session: if the title given is plainly not an IB prescribed title, for example a question the student wrote, say in the overall comment and in a risk that an essay not written on one of the prescribed titles for the session receives no marks, but if the title has been modified and it is still clear which prescribed title it refers to, it is marked against that title and any loss of relevance shows in the mark, so do not say it receives no marks; and still mark the essay as it stands so the feedback is useful.",
  wordLimit: { max: 1600, excludes: "acknowledgements, references (in footnotes, endnotes or in the text), the bibliography, and maps, charts, diagrams, illustrations or tables", stopsAt: true },
};
...
const TOK_EXHIBITION: Rubric = {
  ...
  notes: "Marked holistically against ONE instrument (no sub-criteria), guided by: \"Does the exhibition successfully show how TOK manifests in the world around us?\" Bands: Excellent 9-10 (three objects with specific real-world contexts; links to the ONE selected IA prompt clearly made and well explained; strong justification of each object\u2019s contribution; all, or nearly all, points well supported by appropriate evidence and explicit references to the selected IA prompt). Good 7-8 (three objects and their real-world contexts identified; links to the selected IA prompt explained, though the explanation may lack precision and clarity in parts; a justification of each object's contribution; many points supported by appropriate evidence and references to the prompt). Satisfactory 5-6 (three objects, but their real-world contexts may be vaguely or imprecisely stated; some explanation of the links to the prompt; some justification for including each object; some points supported by evidence and references to the prompt). Basic 3-4 (three objects, but contexts may be implied rather than stated; basic links to the prompt whose explanation is unconvincing and/or unfocused; superficial justification for each object, not supported by appropriate evidence and/or lacking relevance to the prompt; there may be significant repetition across the justifications). Rudimentary 1-2 (three objects, but contexts not stated, or images that are highly generic images of types of object rather than specific real-world objects; links minimal, tenuous or unclear; very little justification; highly descriptive or unsupported assertions). 0 = does not reach these levels or does not use one of the IA prompts. Commentary max 950 words total. " + HOLISTIC_RULE + " Rules: with images and commentaries for only two objects, the mark cannot be higher than six; with an image and commentary for only one object, it cannot be higher than three. State these caps in words (for example \"no higher than three marks\") and never shorten them to the name of a level. The chosen IA prompt must be used exactly as given. If it has been modified but it is still clear which IA prompt it refers to, the exhibition is marked against the original prompt and any loss of relevance shows in the mark, so do not say it receives no marks; only an exhibition clearly not based on one of the 35 IA prompts receives zero.",
  wordLimit: { max: 950, excludes: "text on the objects themselves, acknowledgements, references and the bibliography", stopsAt: true },
};

// ─── English A: Individual Oral (IA, both Language A courses, FA 2021) ───────
const ORAL_NOTES = "The individual oral is spoken, so the paste is an outline or a transcript of a practice oral that uses texts and a global issue different from the assessed oral (the Language A guides let teachers give feedback on such practice orals and on the outline, but not rehearse the actual oral with the student). If the paste presents itself as a rehearsal of the assessed oral, or says it uses the works or the global issue of the assessed oral, say in the overall comment that feedback on a rehearsal of the actual oral is not allowed. If it is a transcript, mark all four criteria on it. If it is an outline or notes, mark A to C on the ideas and structure it shows, and return Criterion D with \"score\": null and a comment saying spoken language cannot be judged from notes; \"max_score\" is then the sum of the criteria you did assess and \"band_range\" is expressed on that total. Do not treat the outline format itself as a weakness.";
...
const MUSIC_IA: Rubric = {
  ...
  notes: "This is Exploring music in context, externally assessed coursework, not the Music internal assessment (Experimenting with music). Portfolio maximum 2,400 words. C1 and C2 judge the creating exercise (its score sits in the written document and/or an audio file) and the performed adaptation (an audio file) themselves, which the pasted text does not let you hear or fully see (the written implications for those exercises are assessed under B2). Return C1 and C2 with \"score\": null and a comment saying they were not assessed because the exercises are not part of the paste; \"max_score\" is the sum of the criteria you did assess and \"band_range\" is expressed on that total. Do not treat the missing exercises as a weakness.",
...
const EXTENDED_ESSAY_2027: Rubric = {
  ...
  wordLimit: { max: 4000, excludes: "headers, citations and references, the bibliography, contents page, tables, equations, charts and diagrams", stopsAt: true },
};
...
const CS_IA_2027: Rubric = {
  ...
  notes: "Documentation capped at 2,000 words (excluding code excerpts, comments and diagrams) plus a video of up to 5 minutes. The appendices must contain the full source code. They are not read for marks, but a solution without it cannot be awarded full marks for the techniques in Criterion D. The appendix is not part of the paste, so do not lower Development for it; say in that comment that the student should check the full source code is in the appendix. The former client requirement is removed. The product and the video are not part of the pasted documentation: mark Development on what the documentation shows, and say in that comment that the product and video were not seen.",
...
- Each criterion score must be an integer from 0 to the stated maximum, except where the notes above say a criterion cannot be assessed from the pasted text: then return "score": null for it.
...
/** How coursework subjects that are not an IA are named where the task is chosen. */
export const COURSEWORK_LABELS: Record<string, string> = {
