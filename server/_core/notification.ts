...
export async function notifyOwner(payload: NotificationPayload): Promise<boolean> {
  ...
  if (!ENV.resendApiKey || !ENV.ownerEmail) {
    console.warn("[Notification] Skipped: RESEND_API_KEY or OWNER_EMAIL not configured");
    return false;
  }
  ...
  try {
    ...
    console.warn("[Notification] Error sending notification email:", err);
    return false;
  }
}
