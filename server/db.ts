...
export async function completePaymentByProviderId(providerPaymentId: string, provider: "coingate") {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getPendingPaymentByProviderIdAndProvider(providerPaymentId: string, provider: "coingate") {
  const db = await getDb();
  if (!db) return undefined;
